...
def get_images_fromtraj(traj_path, args):
...
def run(args):
    ...
    if args.make_concat_gifs:
        image_list = []
        for i, traj in enumerate(trajs):
            image_list.append(get_images_fromtraj(traj, args))
            if len(image_list) == 3:
                make_concat_gifs(args, i, image_list)
                image_list = []
            if i == 20:
                break


def make_concat_gifs(args, i, image_list):
    padded = []
    ...
    npy_to_video(np_unstack(image_rows, 0), args.output_folder + '/batch{}.gif'.format(i))


def npy_to_video(im_list, filename, fps=4):
...
if __name__ == '__main__':
    ...
    parser.add_argument('--make_concat_gifs', default=False, action='store_true')
